...
async def get_msg(userbot, client, bot, sender, edit_id, msg_link, i):
    ...
    if "t.me/c/" or "t.me/b/" in msg_link:
        if "t.me/b/" in msg_link:
            ...
            if not str(msg_link.split("/")[-2]).isnumeric():
                chat = str(msg_link.split("/")[-2])
            else:
        ...
        try:
            ...
            elif msg.media == MessageMediaType.VIDEO and msg.video.mime_type in [
                    "video/mp4",
                    "video/x-matroska",
            ]:
                print("Trying to get metadata")
            ...
            return await get_msg(userbot, client, bot, sender, edit_id,
                                 msg_link, i)
        except Exception as e:
            print(e)
            if ("messages.SendMedia" in str(e) or
                    "SaveBigFilePartRequest" in str(e) or
                    "SendMediaRequest" in str(e) or
                    str(e) == "File size equals to 0 B"):
                try:
                    if msg.media == MessageMediaType.VIDEO and msg.video.mime_type in [
                            "video/mp4",
                            "video/x-matroska",
                    ]:
                        UT = time.time()
                        uploader = await fast_upload(f"{file}", f"{file}", UT,
                                                     bot, edit,
                                                     "**UPLOADING:**")
                        attributes = [
